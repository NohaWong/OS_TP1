...
static int look_for_block(int size)
{
    ...
    while(begin_address < MEMORY_SIZE){
        ...
        if(end_address - begin_address > REALSIZE(sizeof(mem_free_block_t))){
            valid_address=1;
        }
        else{
            int i=0;
            for(i=0; i< REALSIZE(sizeof(mem_free_block_t))-(end_address - begin_address); i++){
                if(memory[end_address+i]==1){
                    break;
                }
            }
            if(end_address - begin_address +i == REALSIZE(sizeof(mem_free_block_t))){
                valid_address=1;
            }   
...
char *memory_alloc(int size)
{
    ...
    int real_size=REALSIZE(size) + REALSIZE(sizeof(mem_used_block_t));
    int i=0;    
    ...
    fprintf(stderr, "ALLOC at : %lu (%d byte(s))\n", 
	    (long unsigned int)(begin_address + REALSIZE(sizeof(mem_used_block_t))), size);
    ...
    return ((char*)&memory[begin_address])+REALSIZE(sizeof(mem_used_block_t));
}

void memory_free(char *p)
{
    int index = p-memory-REALSIZE(sizeof(mem_used_block_t));
